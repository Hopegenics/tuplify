...
WORKDIR /src
COPY ["HG.Tuplify.WebAPI.csproj", "HG.Tuplify.WebAPI/"]
RUN dotnet restore "HG.Tuplify.WebAPI/HG.Tuplify.WebAPI.csproj"

WORKDIR "/src/HG.Tuplify.WebAPI"
COPY . .
RUN dotnet build "HG.Tuplify.WebAPI.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "HG.Tuplify.WebAPI.csproj" \
...
Execute unit tests
```
    cd "HG.Tuplify.WebAPI/test/HG.Tuplify.WebAPI.Tests"
    dotnet test
```

Deploy application
```
    cd "HG.Tuplify.WebAPI/src/HG.Tuplify.WebAPI"
